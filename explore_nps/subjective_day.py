...
# HUMOR 1–10 IMPACTO 1–5
HUMOR_PARAMS = {
    "baseline":   {"hiperreativo": (4.5, 2.0), "dissociativo": (5.5, 1.0), "neutro": (6.5, 1.2)},
    # significa que no baseline, o hiperreativo tem uma média de humor de 4.5 de 10
    # e que essa média pode variar 2.0
...
IMPACTO_PARAMS = {
    ...
    #o impacto do stress no hiperreativo é de 4.5 de 5 (MUITO)
    #e pode variar 0.5
...
# FUNÇÕES DE GERAÇÃO
# NORMALIZA
#transforma qualquer valor em escala de 0 a 1 por causa do np.clip
...
def calc_stress_physio(hr, ibi, rmssd, hr_min, hr_max, ibi_min, ibi_max, rmssd_min, rmssd_max):
    ...
    ibi_n   = 1 - normaliza(ibi,   ibi_min,   ibi_max)   # invertido
...
def calc_stress_subj(humor, emoji_val, impacto):
    h = 1 - normaliza(humor,     1, 10)
    e = 1 - normaliza(emoji_val, 1,  5)
    i = normaliza(impacto,       1,  5)
    return round(0.40 * h + 0.35 * e + 0.25 * i, 4) #1. humor 2.emoji e 3.impacto
...
#filtrar pelos sujeitos que eu quero
...
# dicionário fisiológico: subject → label → valores
fisio_dict = {}
...
# GERAÇÃO DO DIA SIMULADO
# 1 notificação fixa no horário do terapeuta
BASE_DATE = datetime(2025, 1, 13)  # dia simulado
...
for subject in SUJEITOS:
    ...
    fisio  = fisio_dict[subject][label]
    ...
    timestamp = BASE_DATE.replace(hour=hora_fixa, minute=0)
    ...
    rows.append({
        ...
        "HR":               round(fisio["HR"], 2),
        "IBI":              round(fisio["IBI"], 2),
        "RMSSD":            round(fisio["RMSSD"], 2),
        "TEMP":             round(fisio["TEMP"], 2),
        "mood":             humor,
        "emotion_emoji":    emoji,
    ...
    reports[subject]["registros"].append({
        "flag": flag, "divergence": divergence, "insight": insight
    })

# SALVA CSV
df_out = pd.DataFrame(rows)
df_out.to_csv(OUTPUT_PATH / "subjective_day.csv", index=False)

# RELATÓRIO DIÁRIO
print(f"\n{'='*65}")
print(f"  DAILY ANXIETY REPORT — {BASE_DATE.strftime('%Y-%m-%d')}")
print(f"  Horário da Notificação do Terapeuta: {THERAPIST_TIME}")
print(f"{'='*65}")
...
for subject in SUJEITOS:
    ...
    flags      = [r["flag"] for r in regs]
    divs       = [r["divergence"] for r in regs]
    avg_div    = round(sum(divs) / len(divs), 3)
    ...
    print(f"\n  {'─'*55}")
    print(f"  {subject} — profile: {perfil.upper()}")
    print(f"  {'─'*55}")
    print(f"  Respostas no dia:        {total}")
...
print(f"\n{'='*65}")
print(f"  Salvo: explore_nps/subjective_day.csv")
print(f"{'='*65}\n")